...
function PostsTab({ me, posts, setPosts, postsTrash, setPostsTrash, fetchAll }) {
  ...
  useEffect(() => {
    markRead(posts.map(p => p.id));
    // Supabaseのread_byも更新
    const unread = posts.filter(p => !(p.read_by||[]).includes(me));
    unread.forEach(async p => {
      const read_by = [...new Set([...(p.read_by||[]), me])];
      try { await sbFetch(`/posts?id=eq.${p.id}`, { method: "PATCH", body: JSON.stringify({ read_by }), prefer: "return=minimal" }); } catch {}
    });
    if (unread.length > 0) fetchAll();
  }, []);
  ...
  const addPost = async () => {
    ...
    try {
      await sbFetch("/posts", { method: "POST", body: JSON.stringify({ author, content: cnt, image: image||null, likes: {}, comments: [], created_by: author, read_by: [author] }) });
      fetchAll();
    ...
    <div>
      <div style={{ background: "#fafafa", borderRadius: 14, padding: "14px 16px", marginBottom: 20, border: "1px solid #f0f0f0" }}>
        ...
        {image && (
          <div style={{ position: "relative", display: "inline-block", marginTop: 8 }}>
            <img src={image} alt="" style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 10, display: "block" }} />
            <button onClick={() => setImage(null)} style={{ position: "absolute", top: 4, right: 4, background: "rgba(0,0,0,0.5)", border: "none", borderRadius: "50%", color: "#fff", width: 24, height: 24, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
          </div>
        )}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
          <label style={{ cursor: "pointer", color: "#bbb", fontSize: 12, display: "flex", alignItems: "center", gap: 4, border: "1px solid #e0e0e0", borderRadius: 8, padding: "4px 10px" }}>
      ...
      {posts.map(p => (
        <div key={p.id} style={{ borderBottom: "1px solid #f0f0f0", padding: "16px 0", background: isUnread(p) ? "#fff8f5" : "transparent", borderLeft: isUnread(p) ? "3px solid #e05" : "3px solid transparent", paddingLeft: isUnread(p) ? 10 : 0 }}>
          ...
          <div style={{ fontSize: 14, color: "#333", lineHeight: 1.7, paddingLeft: 38, marginBottom: 10 }}>
            {p.content.split("\n").map((line, i) => (
              <span key={i}><Linkify text={line} />{i < p.content.split("\n").length - 1 && <br />}</span>
            ))}
            {p.image && <img src={p.image} alt="" style={{ marginTop: 8, maxWidth: "100%", borderRadius: 10, display: "block" }} />}
          </div>
